...
const ChannelForm = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      let response;
      if (id) {
        response = await axios.post(`/channels/${id}`, data, {
          headers: {
            Authorization: `Bearer ${localStorage.getItem('token')}`
          }
        });
        ...
        response = await axios.post('/channels', data, {
          headers: {
            Authorization: `Bearer ${localStorage.getItem('token')}`
          }
        });
  ...
  return (
    <>
      {loading ? (
        ...
        <div className='my-12 mx-auto md:mx-8 md:ml-[19%] w-[70%]'>
          <div className="ml-4">
